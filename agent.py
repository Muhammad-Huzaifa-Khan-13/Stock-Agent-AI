...
class Agent:
    def __init__(self, i, stock_a_price, stock_b_price, secretary, model):
        ...
        self.stock_a_amount, self.stock_b_amount, self.cash, init_debt = random_init(
        ...
        while retry < max_retry:
            try:
                response = model.generate_content(
                    contents=self.chat_history, generation_config=generation_config
                )
    ...
    def plan_loan(self, date, stock_a_price, stock_b_price, lastday_forum_message):
        ...
        loan_format_check, fail_response, loan = self.secretary.check_loan(
            resp, max_loan
        )  # secretary check loan format
        while not loan_format_check:
            ...
            if try_times > MAX_TRY_TIMES:
                log.logger.warning(
                    "WARNING: Loan format try times > MAX_TRY_TIMES. Skip as no loan today."
                )
                loan = {"loan": "no"}
                break

            resp = self.run_api(
                format_prompt(LOAN_RETRY_PROMPT, {"fail_response": fail_response})
            )
            if resp == "":
                return {"loan": "no"}
            loan_format_check, fail_response, loan = self.secretary.check_loan(
        ...
        if loan["loan"] == "yes":
            loan["repayment_date"] = (
                date + util.LOAN_TYPE_DATE[loan["loan_type"]]
            )  # add loan repayment_date
            self.loans.append(loan)
            # self.action_history[date].append(loan)
            self.cash += loan["amount"]
            log.logger.info(
                "INFO: Agent {} decide to loan: {}".format(self.order, loan)
            )
        else:
    ...
    def plan_stock(self, date, time, stock_a, stock_b, stock_a_deals, stock_b_deals):
        ...
        while not action_format_check:
            ...
            if try_times > MAX_TRY_TIMES:
                log.logger.warning(
                    "WARNING: Action format try times > MAX_TRY_TIMES. Skip as no loan today."
                )
                action = {"action_type": "no"}
                break

            resp = self.run_api(
                format_prompt(BUY_STOCK_RETRY_PROMPT, {"fail_response": fail_response})
            )
            if resp == "":
                return {"action_type": "no"}
            action_format_check, fail_response, action = self.secretary.check_action(
        ...
        if action["action_type"] == "buy":
            ...
            log.logger.info(
                "INFO: Agent {} decide to action: {}".format(self.order, action)
            )
    ...
    def buy_stock(self, stock_name, price, amount):
        ...
        if self.cash < price * amount or stock_name not in ["A", "B"]:
            log.logger.warning(
                "ILLEGAL STOCK BUY BEHAVIOR: remain cash {}".format(self.cash)
            )
            return False
    ...
    def next_day_estimate(self):
        ...
        while not format_check:
            ...
            if try_times > MAX_TRY_TIMES:
                log.logger.warning(
                    "WARNING: Estimation format try times > MAX_TRY_TIMES. Skip as all 'no' today."
                )
                estimate = {
                    "buy_A": "no",
                    "buy_B": "no",
                ...
                break
            resp = self.run_api(
                format_prompt(NEXT_DAY_ESTIMATE_RETRY, {"fail_response": fail_response})
            )
            if resp == "":
                return {
                    "buy_A": "no",
