        ...
        if action["action_type"] == "buy":
            for sell_action in stock_deals["sell"][:]:
                if action["price"] == sell_action["price"]:
                    ...
                    get_agent(all_agents, action["agent"]).buy_stock(
                        stock.name, close_amount, action["price"]
                    )
                    if not sell_action["agent"] == -1:  # B发行
                        get_agent(all_agents, sell_action["agent"]).sell_stock(
                            stock.name, close_amount, action["price"]
                        )
                    stock.add_session_deal(
            ...
            for buy_action in stock_deals["buy"][:]:
                if action["price"] == buy_action["price"]:
                    ...
                    get_agent(all_agents, action["agent"]).sell_stock(
                        stock.name, close_amount, action["price"]
                    )
                    get_agent(all_agents, buy_action["agent"]).buy_stock(
                        stock.name, close_amount, action["price"]
                    )
                    stock.add_session_deal(
                        {"price": action["price"], "amount": close_amount}
...
def simulation(args):
    ...
    for i in range(0, util.AGENTS_NUM):  # agents start from 0, -1 refers to admin
        agent = Agent(
            i, stock_a.get_price(), stock_b.get_price(), secretary, args.model
        )
        all_agents.append(agent)
        log.logger.debug(
            "cash: {}, stock a: {}, stock b:{}, debt: {}".format(
                agent.cash, agent.stock_a_amount, agent.stock_b_amount, agent.loans
            )
        )
    ...
    for date in range(1, util.TOTAL_DATE + 1):
        ...
        daily_agent_records = []
        for agent in all_agents:
            loan = agent.plan_loan(
                date, stock_a.get_price(), stock_b.get_price(), last_day_forum_message
            )
        ...
        for session in range(1, util.TOTAL_SESSION + 1):
            ...
            for i in sequence:
                ...
                action = agent.plan_stock(
                    date, session, stock_a, stock_b, stock_a_deals, stock_b_deals
                )
                ...
                if not action["action_type"] == "no":
                    if action["stock"] == "A":
                        handle_action(
                            action, stock_a_deals, all_agents, stock_a, session
                        )
                    else:
                        handle_action(
                            action, stock_b_deals, all_agents, stock_b, session
                        )
        ...
        for idx, agent in enumerate(all_agents):
            ...
            log.logger.info(
                "Agent {} tomorrow estimation: {}".format(agent.order, estimation)
            )
